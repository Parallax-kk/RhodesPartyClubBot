...
async def handle_event_detection(message, change):
  if all(keyword in message.content
         for keyword in ["趣旨：", "期限：", "日程：", "場所：", "予算：", "人数：", "特記："]):
    ...
    if add_google_calendar_flag:
      mm_dd_match = match_date_pattern(MM_DD_PATTERN,
                                       remove_brackets(message.channel.name))

      if not mm_dd_match:
        await message.response.send_message(MSG_NEED_CHANGE_EVENT_TITLE,
                                            ephemeral=True)
        return
      ...
      datail = message.content.replace('\n', '%0D%0A')
      url = (
...
# Send Notificationコマンド
@tree.command(name="send_notification", description="みんなにイベント通知する")
@discord.app_commands.default_permissions(administrator=True)
@discord.app_commands.guilds(GUILD_ID)
async def send_notification(ctx: discord.Interaction):
  if ctx.guild.id == int(GUILD_ID):
